...
from preprocessing.preprocessing import preprocess_raw_data
from preprocessing.util import draw_bboxes
from preprocessing.voc2yolo import voc2yolo_convert
from utils.datasets import create_dataloader
from utils.general import coco80_to_coco91_class, check_dataset, check_file, check_img_size, check_requirements, \
    box_iou, non_max_suppression, scale_coords, xyxy2xywh, xywh2xyxy, set_logging, increment_path, colorstr
from utils.json_utils import JsonTemplate
...
def draw_vis_img(path, predn, summary_path, result_summary, load_gt=False, filter_threshold=0.5):
    ...
    pred_img = draw_bboxes(origin_img,
                           {'bboxes': selected_bbox[:, :4].cpu().numpy(), 'name': selected_bbox[:, 4].cpu().numpy()},
                           color=(0, 255, 0))
    if load_gt:
    ...
    result_summary.append(
        {'img': raw_name, 'img_meta': '%s.%s' % (raw_name, 'json'), 'vis_img': '%s_vis.jpg' % raw_name})


def test(opt, data, weights=None, batch_size=32, imgsz=640, conf_thres=0.001, iou_thres=0.6,  # for NMS
         iou_overlap=0.1,  # for mAP0.5
         save_json=False, single_cls=True, augment=False, verbose=False, model=None, dataloader=None, save_dir=Path(''),
         # for saving images
         save_txt=False,  # for auto-labelling
         save_hybrid=False,  # for hybrid auto-labelling
         save_conf=False,  # save auto-label confidences
         plots=True, wandb_logger=None, compute_loss=None, half_precision=True, trace=False, is_coco=False,
         v5_metric=False, use_thread_pool=True):
    ...
    iouv = torch.linspace(iou_overlap, 1.0, 10).to(device)  # iou vector for mAP@0.5:0.95
    ...
    if not training:
        ...
        dataloader = \
            create_dataloader(path, imgsz, batch_size, gs, opt, pad=0.5, rect=True, prefix=colorstr(f'{path}: '))[0]
    ...
    s = ('%20s' + '%12s' * 6) % (
        'Class', 'Images', 'Labels', 'P', 'R', 'mAP@%.f' % iou_overlap, 'mAP@%.1f:.95' % iou_overlap)
    ...
    if not training and opt.greed_search:
        ...
        for threshold in tqdm(range(opt.greed_search_range[0], opt.greed_search_range[1], opt.greed_search_range[2]),
            ...
            if f1_score > best_f1:
                ...
                Logger.info(
                    f'threshold:{threshold}, tp:{nb_tp}, fp:{nb_fp}, fn:{nb_fn}, gt:{nb_gt}, acc:{round(acc, 2)}, '
        ...
        p_bar = tqdm(total=len(json_templates.keys()),
                     desc="Export json with conf threshold:%s" % max(0.1, best_threshold))
        ...
        with open((save_dir / "평가결과.txt").as_posix(), 'w', encoding='utf-8') as f:
            content = str({'F1-Score': best_f1, 'accuracy': best_acc, 'mAP': best_map})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', nargs='+', type=str,
                        default='/Strawberry/yolov5/output/train/exp_904aug_ndata/weights/best.pt',
                        help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='/home/jovyan/project/jbnu/pwd2022_params/pwd.yaml', help='*.data '
                                                                                                              'path')
    parser.add_argument('--input', type=str, help='json input folder path')
    ...
    opt.device, opt.weights, opt.iou_overlap, opt.conf_thres, opt.project = opt.gpu_num, opt.model_path, opt.IoU, opt.threshold, opt.output
    ...
    if opt.task in ('val', 'test'):  # run normally
        test(opt=opt, data=opt.data, weights=opt.weights, batch_size=opt.batch_size, imgsz=opt.img_size,
             conf_thres=opt.conf_thres, iou_overlap=opt.iou_overlap, save_json=opt.save_json, single_cls=opt.single_cls,
             augment=opt.augment, verbose=opt.verbose, save_txt=opt.save_txt | opt.save_hybrid,
             save_hybrid=opt.save_hybrid, save_conf=opt.save_conf, trace=not opt.no_trace, v5_metric=opt.v5_metric)

    elif opt.task == 'speed':  # speed benchmarks
        for w in opt.weights:
            test(opt, opt.data, w, opt.batch_size, opt.img_size, 0.25, 0.45, save_json=False, plots=False,
